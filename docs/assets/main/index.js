...
System.register("chunks:///_virtual/GameController.ts", ['./rollupPluginModLoBabelHelpers.js', 'cc'], function (exports) {
  var _createForOfIteratorHelperLoose, cclegacy, input, Input;
  return {
    setters: [function (module) {
    ...
    execute: function () {
      ...
      var GameController = exports('GameController', /*#__PURE__*/function () {
        ...
        _proto.enqueueTrafficSpawnRequests = function enqueueTrafficSpawnRequests() {
          var _this = this;
          var requests = this.manager.model.drainTrafficSpawnRequests();
          if (requests.length === 0) {
            return;
          }
          var waitingCount = this.manager.model.waitingPassengers.length + this.pendingPassengerSpawns.length;
          var maxWaitingPassengers = this.manager.model.currentWaitingPassengerLimit;
          if (waitingCount >= maxWaitingPassengers) {
            return;
          }
          var room = maxWaitingPassengers - waitingCount;
          var accepted = requests.slice(0, room);
          var countsByOrigin = new Map();
          accepted.forEach(function (request) {
            var _countsByOrigin$get;
            _this.pendingPassengerSpawns.push({
              origin: request.originFloor,
              destination: request.destinationFloor
            });
            countsByOrigin.set(request.originFloor, ((_countsByOrigin$get = countsByOrigin.get(request.originFloor)) != null ? _countsByOrigin$get : 0) + 1);
          });
          for (var _iterator = _createForOfIteratorHelperLoose(countsByOrigin.entries()), _step; !(_step = _iterator()).done;) {
            var _step$value = _step.value,
              origin = _step$value[0],
              count = _step$value[1];
...
System.register("chunks:///_virtual/GameModel.ts", ['./rollupPluginModLoBabelHelpers.js', 'cc', './GameTypes.ts'], function (exports) {
  ...
  return {
    ...
    execute: function () {
      ...
      var START_TIME = exports('START_TIME', 6 * 60);
      var TIME_SCALE = exports('TIME_SCALE', 1);
      var SNAPSHOT_VERSION = 2;
      var MIN_FLOOR = -2;
      var INITIAL_UNLOCKED_FLOORS = 11;
      var DEFAULT_MAX_FLOOR = 10;
      ...
      var ELEVATOR_CAPACITY = 5;
      var DELIVERY_SCORE_BASE = 10;
      var DIFFICULTY_STAGES = [{
        maxLevel: 1,
        timeScale: 1,
        targetDeliveries: 12,
        ambientFirstDelaySeconds: 3,
        ambientMinIntervalSeconds: 8,
        ambientMaxIntervalSeconds: 13,
        ambientMin: 1,
        ambientMax: 1,
      ...
      var FLOOR_RUSH_CAPS = [{
        ...
        maxUnlockedFloors: 11,
        rushCap: 10
      }, {
        maxUnlockedFloors: 14,
        rushCap: 14
      }, {
        maxUnlockedFloors: 18,
        rushCap: 18
      }, {
        maxUnlockedFloors: Number.MAX_SAFE_INTEGER,
        rushCap: 24
      }];
      var RUSH_ROUTE_MULTIPLIERS = {
        'ground-office': 1,
        'parking-office': 0.8,
      ...
      var GameModel = exports('GameModel', /*#__PURE__*/function () {
        function GameModel() {
          ...
          this.trafficSpawnRequests = [];
          this.ambientTrafficTimer = 0;
          this.nextAmbientTrafficDelaySeconds = DIFFICULTY_STAGES[0].ambientFirstDelaySeconds;
          this.smallQueueTimer = 0;
          this.floorTypeOverrides = new Map([[-2, 'parking'], [-1, 'parking'], [0, 'ground'], [1, 'restaurant'], [2, 'rest']]);
          this.rushEvents = [{
            time: 9 * 60,
            warningLeadTime: 180,
            fromType: 'ground',
        ...
        _proto.restore = function restore(snapshot) {
          var _snapshot$upgrades, _this$economy, _this$economy$score, _this$economy2, _this$economy2$bestSc, _this$progress, _this$progress$starte, _this$progress2, _this$progress2$gameT;
          ...
          (_this$progress2$gameT = (_this$progress2 = this.progress).gameTime) != null ? _this$progress2$gameT : _this$progress2.gameTime = START_TIME;
        ...
        _proto.createPassenger = function createPassenger(originFloor, destinationFloor) {
          var maxPatience = this.maxPassengerPatience;
          var passenger = {
        ...
        _proto.drainTrafficSpawnRequests = function drainTrafficSpawnRequests() {
          return this.trafficSpawnRequests.splice(0);
        };
        _proto.getRenderableFloors = function getRenderableFloors() {
          var maxFloor = Math.max(DEFAULT_MAX_FLOOR, this.maxUnlockedFloor);
          var floors = [];
          for (var floor = MIN_FLOOR; floor <= maxFloor; floor += 1) {
            floors.push(floor);
          }
          return floors;
        };
        _proto.isFloorUnlocked = function isFloorUnlocked(floor) {
          return floor >= MIN_FLOOR && floor <= this.maxUnlockedFloor;
        };
        _proto.getFloorType = function getFloorType(floor) {
          var _this$floorTypeOverri;
          return (_this$floorTypeOverri = this.floorTypeOverrides.get(floor)) != null ? _this$floorTypeOverri : 'office';
        };
        _proto.getFloorsByType = function getFloorsByType(type) {
          var _this = this;
          return this.getRenderableFloors().filter(function (floor) {
            return _this.isFloorUnlocked(floor) && _this.getFloorType(floor) === type;
          });
        };
        _proto.getUpcomingRushEvents = function getUpcomingRushEvents(limit) {
          var _this2 = this;
          if (limit === void 0) {
            limit = 2;
          }
          return this.rushEvents.filter(function (event) {
            return !event.triggered;
          }).filter(function (event) {
            return _this2.getRushAmount(event) > 0;
          }).map(function (event) {
            return _extends({}, event, {
              remainingMinutes: event.time - _this2.progress.gameTime
            });
          }).filter(function (event) {
            return event.remainingMinutes >= 0 && event.remainingMinutes <= event.warningLeadTime;
          }).sort(function (left, right) {
        ...
        _proto.boardAtElevator = function boardAtElevator(elevatorIndex) {
          ...
          var overflowIndex = this.elevators.findIndex(function (other, index) {
            return index !== elevatorIndex && other.currentFloor === elevator.currentFloor && Math.abs(other.position - elevator.position) < 0.001 && other.doorOpen && _this3.unloadingQueues[index].length === 0 && !_this3.isElevatorFullAt(index);
          });
          if (overflowIndex < 0) {
            return boarded;
          }
        ...
        _proto.update = function update(deltaTime) {
          ...
          var gameMinutes = deltaTime * this.currentTimeScale;
          ...
          this.elevators.forEach(function (_elevator, index) {
            _this4.updateUnloading(deltaTime, index);
            _this4.updateBoarding(deltaTime, index);
            _this4.updateElevator(deltaTime, index);
          });
        ...
        _proto.startNewGame = function startNewGame() {
          ...
          this.progress.unlockedFloors = INITIAL_UNLOCKED_FLOORS;
          this.progress.elapsedSeconds = 0;
          this.progress.gameTime = START_TIME;
          this.progress.started = false;
          this.progress.completed = false;
        ...
        _proto.updatePatience = function updatePatience(deltaTime, state) {
          for (var _iterator = _createForOfIteratorHelperLoose(this.passengers.filter(function (candidate) {
        ...
        _proto.updateTraffic = function updateTraffic(gameMinutes, deltaTime) {
          var _this5 = this;
          var spawnedRush = false;
          this.rushEvents.forEach(function (event) {
            if (event.triggered || _this5.progress.gameTime < event.time) {
              return;
            }
            _this5.enqueueTrafficPassengers(event.fromType, event.toType, _this5.getRushAmount(event));
          ...
          if (this.ambientTrafficTimer >= this.nextAmbientTrafficDelaySeconds) {
            this.ambientTrafficTimer = 0;
            this.nextAmbientTrafficDelaySeconds = this.rollAmbientTrafficDelaySeconds(difficulty);
            this.enqueueRandomLowTrafficPassengers(this.rollAmount(difficulty.ambientMin, difficulty.ambientMax));
          }
          this.smallQueueTimer += deltaTime;
          if (this.smallQueueTimer >= difficulty.smallQueueIntervalSeconds) {
            this.smallQueueTimer %= difficulty.smallQueueIntervalSeconds;
            this.enqueueRandomLowTrafficPassengers(this.rollAmount(difficulty.smallQueueMin, difficulty.smallQueueMax));
          }
        };
        _proto.rollAmbientTrafficDelaySeconds = function rollAmbientTrafficDelaySeconds(difficulty) {
          var range = difficulty.ambientMaxIntervalSeconds - difficulty.ambientMinIntervalSeconds;
          return difficulty.ambientMinIntervalSeconds + Math.random() * Math.max(0, range);
        };
        _proto.rollAmount = function rollAmount(min, max) {
          return min + Math.floor(Math.random() * (max - min + 1));
        };
        _proto.getRushAmount = function getRushAmount(event) {
          var _RUSH_ROUTE_MULTIPLIE;
          var difficulty = this.currentDifficulty;
          if (!difficulty.parkingTrafficEnabled && (event.fromType === 'parking' || event.toType === 'parking')) {
            return 0;
          }
          var routeKey = event.fromType + "-" + event.toType;
          var multiplier = (_RUSH_ROUTE_MULTIPLIE = RUSH_ROUTE_MULTIPLIERS[routeKey]) != null ? _RUSH_ROUTE_MULTIPLIE : 0.6;
          var cap = Math.min(difficulty.rushCap, this.floorRushCap);
          var amount = Math.round(cap * multiplier);
          return Math.max(0, Math.min(cap, amount));
        };
        _proto.enqueueTrafficPassengers = function enqueueTrafficPassengers(fromType, toType, amount) {
          var fromFloors = this.getFloorsByType(fromType);
          var toFloors = this.getFloorsByType(toType);
          if (fromFloors.length === 0 || toFloors.length === 0) {
            return;
          }
          for (var index = 0; index < amount; index += 1) {
            var originFloor = fromFloors.length === 1 ? fromFloors[0] : fromFloors[index % fromFloors.length];
            var destinationFloor = toFloors[Math.floor(Math.random() * toFloors.length)];
            ...
            this.trafficSpawnRequests.push({
              originFloor: originFloor,
              destinationFloor: destinationFloor
            });
          }
        };
        _proto.enqueueRandomLowTrafficPassengers = function enqueueRandomLowTrafficPassengers(amount) {
          var _this6 = this;
          var floors = this.getRenderableFloors().filter(function (floor) {
            return _this6.isFloorUnlocked(floor);
          });
          if (floors.length < 2) {
            return;
          }
          for (var index = 0; index < amount; index += 1) {
            var originFloor = floors[Math.floor(Math.random() * floors.length)];
            var destinationFloor = floors[Math.floor(Math.random() * floors.length)];
            if (destinationFloor === originFloor) {
        ...
        _proto.updatePatienceWarnings = function updatePatienceWarnings(deltaTime) {
          ...
          warningFloors.forEach(function (floor) {
            return _this7.warningEvents.push({
              floor: floor
            });
          });
        };
        ...
        _proto.updateBoarding = function updateBoarding(deltaTime, elevatorIndex) {
          ...
          while (this.boardingTimers[elevatorIndex] >= BOARDING_INTERVAL && boardingQueue.length > 0) {
            ...
            this.resetPassengerPatience(_passenger2);
        ...
        _proto.beginUnloadingAtCurrentFloor = function beginUnloadingAtCurrentFloor(elevatorIndex, arrivalDirection) {
          ...
          var deliveredIds = elevator.passengers.filter(function (id) {
            var _this8$getPassenger;
            return ((_this8$getPassenger = _this8.getPassenger(id)) == null ? void 0 : _this8$getPassenger.destinationFloor) === elevator.currentFloor;
          });
          if (deliveredIds.length === 0) {
            return false;
          }
          deliveredIds.forEach(function (id) {
            var passenger = _this8.getPassenger(id);
            if (passenger) {
        ...
        _proto.updateUnloading = function updateUnloading(deltaTime, elevatorIndex) {
          ...
          var _loop = function _loop() {
            _this9.unloadingTimers[elevatorIndex] -= UNLOADING_INTERVAL;
            var passengerId = unloadingQueue.shift();
            var passenger = passengerId === undefined ? undefined : _this9.getPassenger(passengerId);
            if (!passenger || passenger.state !== PassengerState.Exiting) {
              return 1; // continue
            }

            passenger.state = PassengerState.Delivered;
            var elevator = _this9.elevators[elevatorIndex];
            elevator.passengers = elevator.passengers.filter(function (id) {
              return id !== passenger.id;
            });
            var patienceRatio = passenger.patience / passenger.maxPatience;
            var deliveryMultiplier = patienceRatio >= 0.8 ? 3 : patienceRatio >= 0.5 ? 2 : 1;
            _this9.economy.multiplier = deliveryMultiplier;
            _this9.economy.multiplierProgress = 0;
            _this9.economy.delivered += 1;
            _this9.economy.coins += deliveryMultiplier;
            var scoreGain = DELIVERY_SCORE_BASE * deliveryMultiplier;
            _this9.economy.score += scoreGain;
            _this9.economy.bestScore = Math.max(_this9.economy.bestScore, _this9.economy.score);
            _this9.stopDeliveredCounts[elevatorIndex] += 1;
            _this9.deliveredEvents.push({
              passengerId: passenger.id,
              floor: elevator.currentFloor,
              multiplier: deliveryMultiplier,
              scoreGain: scoreGain,
              stopDeliveredCount: _this9.stopDeliveredCounts[elevatorIndex],
              totalDelivered: _this9.economy.delivered,
              elevatorIndex: elevatorIndex
            });
          };
        ...
        _proto.resetPassengerPatience = function resetPassengerPatience(passenger) {
        ...
        _proto.boardPassengersAtCurrentFloor = function boardPassengersAtCurrentFloor(elevatorIndex, predicate) {
          ...
          boarding.forEach(function (passenger) {
            ...
            _this10.boardingQueues[elevatorIndex].push(passenger.id);
          });
        ...
        _proto.enqueueStops = function enqueueStops(floors, elevatorIndex) {
          ...
          floors.forEach(function (floor) {
            if (!_this11.isFloorUnlocked(floor)) {
              return;
            }
        ...
        _proto.resetTraffic = function resetTraffic() {
          this.rushEvents.forEach(function (event) {
            event.triggered = false;
          });
          this.trafficSpawnRequests.length = 0;
          this.ambientTrafficTimer = 0;
          this.nextAmbientTrafficDelaySeconds = this.currentDifficulty.ambientFirstDelaySeconds;
          this.smallQueueTimer = 0;
        };
        ...
        _createClass(GameModel, [{
          ...
          get: function get() {
            var _this12 = this;
            var waitingWarningFloors = this.waitingPassengers.filter(function (passenger) {
            ...
            var elevatorWarningFloors = this.elevators.filter(function (elevator) {
              return elevator.passengers.some(function (id) {
                var passenger = _this12.getPassenger(id);
                return !!passenger && passenger.waitElapsed / passenger.maxPatience >= PATIENCE_WARNING_RATIO;
              });
            }).map(function (elevator) {
              return elevator.currentFloor;
            });
            return [].concat(new Set([].concat(waitingWarningFloors, elevatorWarningFloors)));
          }
        }, {
          ...
          key: "maxUnlockedFloor",
          get: function get() {
            return this.progress.unlockedFloors - 1;
          }
        }, {
          key: "currentTimeScale",
          get: function get() {
            return this.currentDifficulty.timeScale;
          }
        }, {
          key: "currentWaitingPassengerLimit",
          get: function get() {
            return this.currentDifficulty.maxWaitingPassengers;
          }
        }, {
          ...
          get: function get() {
            var _DIFFICULTY_STAGES$fi,
              _this13 = this;
            return (_DIFFICULTY_STAGES$fi = DIFFICULTY_STAGES.find(function (stage) {
              return _this13.progress.level <= stage.maxLevel;
            })) != null ? _DIFFICULTY_STAGES$fi : DIFFICULTY_STAGES[DIFFICULTY_STAGES.length - 1];
          }
        }, {
          key: "floorRushCap",
          get: function get() {
            var _this14 = this,
              _floorCap$rushCap;
            var floorCap = FLOOR_RUSH_CAPS.find(function (entry) {
              return _this14.progress.unlockedFloors <= entry.maxUnlockedFloors;
            });
            return (_floorCap$rushCap = floorCap == null ? void 0 : floorCap.rushCap) != null ? _floorCap$rushCap : FLOOR_RUSH_CAPS[FLOOR_RUSH_CAPS.length - 1].rushCap;
          }
        }, {
          key: "elevatorSpeed",
          get: function get() {
...
System.register("chunks:///_virtual/GameView.ts", ['./rollupPluginModLoBabelHelpers.js', 'cc', './GameTypes.ts'], function (exports) {
  var _createForOfIteratorHelperLoose, _createClass, _extends, cclegacy, Color, Vec3, UITransform, Label, Node, Layers, Sprite, resources, SpriteFrame, Graphics, UpgradeType, ElevatorDirection;
  return {
    setters: [function (module) {
      _createForOfIteratorHelperLoose = module.createForOfIteratorHelperLoose;
      _createClass = module.createClass;
    ...
    execute: function () {
      ...
      var FLOOR_COLORS = [new Color(24, 154, 181, 255), new Color(75, 124, 214, 255), new Color(106, 49, 164, 255), new Color(240, 151, 0, 255), new Color(123, 181, 14, 255), new Color(27, 105, 198, 255), new Color(205, 42, 47, 255), new Color(231, 92, 137, 255), new Color(64, 131, 102, 255), new Color(224, 104, 50, 255), new Color(80, 82, 190, 255), new Color(142, 89, 42, 255), new Color(38, 165, 135, 255), new Color(84, 128, 63, 255), new Color(177, 74, 54, 255), new Color(58, 142, 196, 255)];
      ...
      var GameView = exports('GameView', /*#__PURE__*/function () {
        function GameView(parent, events) {
          ...
          this.labels = {};
          this.floorYs = new Map();
          this.clickableFloorValues = [];
        ...
        _proto.floorAt = function floorAt(position) {
          ...
          for (var _iterator = _createForOfIteratorHelperLoose(this.clickableFloorValues), _step; !(_step = _iterator()).done;) {
            var floor = _step.value;
            var floorY = this.floorYs.get(floor);
            if (floorY === undefined) {
              continue;
            }
            if (Math.abs(position.y - floorY) <= this.floorHitHalfHeight && position.x > -340 && position.x < 340) {
              return floor;
            }
          }
          return null;
        ...
        _proto.isNewGameButton = function isNewGameButton(position) {
        ...
        _proto.drawRushWarnings = function drawRushWarnings(model) {
          var _this2 = this;
          var warnings = model.getUpcomingRushEvents(2);
          warnings.forEach(function (warning, index) {
            var y = 560 - index * 34;
            var x = -65;
            var fromColor = FLOOR_TYPE_COLORS[warning.fromType];
            var toColor = FLOOR_TYPE_COLORS[warning.toType];
            _this2.graphics.fillColor = fromColor;
            _this2.graphics.circle(x, y, 8);
            _this2.graphics.fill();
            _this2.graphics.fillColor = toColor;
            _this2.graphics.rect(x + 38, y - 8, 16, 16);
            _this2.graphics.fill();
            _this2.drawText("rush-warning-" + index, _this2.formatRushCountdown(warning.remainingMinutes) + "  " + _this2.floorTypeLabel(warning.fromType) + " \u2192 " + _this2.floorTypeLabel(warning.toType), x + 66, y, 17, INK, 250);
          });
        };
        _proto.drawTower = function drawTower(model) {
          var _this3 = this;
          var floors = model.getRenderableFloors();
          var floorCount = Math.max(MIN_VISIBLE_FLOORS, floors.length);
          ...
          for (var index = 0; index < floorCount; index += 1) {
            var _floors$index;
            var floor = (_floors$index = floors[index]) != null ? _floors$index : model.minFloor + index;
            var y = bottomY + index * floorGap;
            var unlocked = model.isFloorUnlocked(floor);
          ...
          model.elevators.forEach(function (elevator, index) {
            var elevatorY = bottomY + (elevator.position - model.minFloor) * floorGap;
            if (elevatorY > TOWER_BOTTOM - 70 && elevatorY < TOWER_TOP + 70) {
              _this3.drawCabin(model, elevator, index, elevatorXs[index], elevatorY, shaftWidth);
            }
          });
        };
        ...
        _proto.drawOffscreenPassengerArrows = function drawOffscreenPassengerArrows(model) {
          var _this4 = this;
          if (!model.progress.started || model.progress.completed || model.progress.failed) {
            return;
          }
          var hasAbove = false;
          var hasBelow = false;
          var waitingFloors = new Set(model.waitingPassengers.map(function (passenger) {
            return passenger.originFloor;
          }));
          waitingFloors.forEach(function (floor) {
            var y = _this4.floorYs.get(floor);
            if (y === undefined) {
              return;
            }
            if (y > TOWER_TOP) {
          ...
          var pulse = (Math.sin(model.progress.elapsedSeconds * Math.PI * 4) + 1) * 0.5;
          var color = new Color(247, 243, 237, 150 + Math.round(pulse * 105));
          if (hasAbove) {
        ...
        _proto.drawOffscreenArrow = function drawOffscreenArrow(x, y, up, color) {
        ...
        _proto.drawFloorMarker = function drawFloorMarker(model, floor, y, unlocked) {
          var floorLabel = this.formatFloorLabel(floor);
          var color = this.floorColor(model, floor);
        ...
        _proto.drawOfficeDetails = function drawOfficeDetails(model, floor, y, floorGap, unlocked) {
          ...
          var companyNames = ['星火创意', '启明咨询', '智云数据', '未来科技', '国际会议', '云帆金融'];
          var typeNames = {
            ground: '大堂接待',
            parking: '地下停车',
            restaurant: '员工餐厅',
          ...
          this.drawText("company-" + floor, unlocked ? typeNames[model.getFloorType(floor)] : '待招商楼层', -112, y + 2, 17, unlocked ? PAPER : MUTED, 120);
        ...
        _proto.drawPassengers = function drawPassengers(model, floor, y) {
          ...
          passengers.forEach(function (passenger, index) {
            ...
            var x = 75 - index * 40;
            _this5.drawPassenger(passenger, _this5.getPassengerX(model, passenger, x), y, model.getPassengerWaitProgress(passenger), model.shouldShowPassengerTimer(passenger));
          });
        };
        ...
        _proto.drawPassenger = function drawPassenger(passenger, x, y, waitProgress, showTimer) {
          var clothingColor = this.floorColorByFloor(passenger.destinationFloor);
        ...
        _proto.drawCabin = function drawCabin(model, elevator, elevatorIndex, x, y, width) {
          ...
          elevator.passengers.slice(0, elevator.capacity).forEach(function (id, index) {
            ...
            if (passenger) {
              ...
              var iconX = x + 17 + column * 22;
              var iconY = y - 7 - row * 23;
              _this6.graphics.fillColor = _this6.floorColor(model, passenger.destinationFloor);
              _this6.graphics.roundRect(iconX, iconY, 14, 18, 3);
              _this6.graphics.fill();
              if (model.shouldShowPassengerTimer(passenger)) {
                var waitProgress = model.getPassengerWaitProgress(passenger);
                _this6.graphics.strokeColor = waitProgress >= 0.75 ? DANGER : new Color(225, 220, 211, 180);
                _this6.graphics.lineWidth = 2;
                _this6.graphics.arc(iconX + 7, iconY + 9, 13, Math.PI / 2, Math.PI / 2 + Math.PI * 2 * waitProgress, false);
                _this6.graphics.stroke();
              }
            }
          });
        };
        _proto.drawDeliveryFeedback = function drawDeliveryFeedback(model) {
          ...
          var floorY = this.floorYs.get(feedback.floor);
          ...
          if (feedback.multiplier > 1) {
            var _feedback$elevatorInd;
            var elevatorIndex = (_feedback$elevatorInd = feedback.elevatorIndex) != null ? _feedback$elevatorInd : 0;
            var badgeX = elevatorIndex === 0 ? 235 : 120;
            this.graphics.fillColor = new Color(22, 23, 25, 230);
            this.graphics.roundRect(badgeX, floorY + 78, 58, 32, 6);
            this.graphics.fill();
            this.drawText('deliveryMultiplier', feedback.multiplier + "X", badgeX + 11, floorY + 94, 19, GOLD, 38);
          } else if (this.labels.deliveryMultiplier) {
            this.labels.deliveryMultiplier.node.active = false;
          }
        };
        _proto.drawScoreBonusFeedback = function drawScoreBonusFeedback() {
          var label = this.labels.scoreBonus;
          var feedback = this.deliveryFeedback;
          if (!label || !feedback) {
            if (label) {
              label.node.active = false;
          ...
          var elapsed = Date.now() - feedback.startedAt;
          if (elapsed > 1000) {
            label.node.active = false;
            return;
          }
          ...
          label.string = feedback.multiplier > 1 ? feedback.multiplier + "X +" + feedback.scoreGain : "+" + feedback.scoreGain;
        };
        ...
        _proto.drawCompletion = function drawCompletion(model) {
          ...
          this.drawUpgradeCard(-290, UpgradeType.Capacity, '容量', "\u5355\u68AF\u4E0A\u9650 5\n\u5F53\u524D " + model.elevator.capacity);
        ...
        _proto.setUpgradeLabelsActive = function setUpgradeLabelsActive(active) {
          ...
          [UpgradeType.Capacity, UpgradeType.Speed, UpgradeType.Patience].forEach(function (key) {
            ["upgrade-" + key + "-title", "upgrade-" + key + "-desc"].forEach(function (labelKey) {
              if (_this7.labels[labelKey]) {
                _this7.labels[labelKey].node.active = active;
              }
            });
          });
        };
        ...
        _proto.setMenuLabelsActive = function setMenuLabelsActive(active) {
          var _this8 = this;
          ['menu-title', 'menu-progress', 'menu-save', 'menu-new-game', 'menu-close'].forEach(function (key) {
            if (_this8.labels[key]) {
              _this8.labels[key].node.active = active;
            }
          });
        };
        ...
        _proto.hideDynamicTowerLabels = function hideDynamicTowerLabels() {
          Object.entries(this.labels).forEach(function (_ref2) {
            var key = _ref2[0],
              label = _ref2[1];
            if (key.startsWith('shaft-') || key.startsWith('floor-') || key.startsWith('company-') || key.startsWith('cabin-target-') || key.startsWith('cabin-passenger-') || key.startsWith('queue-increase-') || key.startsWith('rush-warning-') || key.startsWith('start-') || key === 'deliveryCount' || key === 'deliveryMultiplier') {
              label.node.active = false;
            }
          });
        };
        _proto.floorColor = function floorColor(model, floor) {
          var index = model.getRenderableFloors().indexOf(floor);
          return FLOOR_COLORS[Math.max(0, index) % FLOOR_COLORS.length];
        };
        _proto.floorColorByFloor = function floorColorByFloor(floor) {
          var index = floor + 2;
          return FLOOR_COLORS[Math.max(0, index) % FLOOR_COLORS.length];
        };
        ...
        _proto.formatGameTime = function formatGameTime(gameMinutes) {
          var normalized = Math.max(0, Math.floor(gameMinutes));
          var hours = Math.floor(normalized / 60) % 24;
          var minutes = normalized % 60;
          return String(hours).padStart(2, '0') + ":" + String(minutes).padStart(2, '0');
        };
        _proto.formatRushCountdown = function formatRushCountdown(remainingMinutes) {
          var minutes = Math.ceil(Math.max(0, remainingMinutes));
          if (minutes <= 10) {
            return '即将到来';
          }
        ...
        _proto.floorTypeLabel = function floorTypeLabel(type) {
          var labels = {
            ground: 'G层',
            parking: '停车场',
            office: '办公层',
